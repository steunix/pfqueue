...
int pfb_init() {
	...
	syslog ( LOGLEVEL, "pfqueue postfix2 backend: initialized" );
	return PFBE_OK;
}

void pfb_set_path(const char* p) {
	strncpy ( pfb_conf.backend_path, p, sizeof(pfb_conf.backend_path) );
}
...
int pfb_setup( struct msg_t *qptr1, struct be_msg_t *qptr2 ) {
	...
	if ( strlen(pfb_conf.version) ) {
		switch ( pfb_conf.version[0] ) {
		case '0':
			pf_version = PF_VERSION_20;
			break;
		case '1':
			pf_version = PF_VERSION_21;
			break;
		case '2':
	...
	if ( pf_version == -1 ) {
		...
		p = popen ( pconf, "r" );
		if ( !p ) {
			syslog ( LOGLEVEL, "pfqueue postfix2 backend: cannot guess postfix version, using 2.2 as default" );
			sprintf ( buf, "2.2" );
		} else
		freadl ( p, buf, sizeof(buf) );
		if ( !strncmp ( buf, "2.0", 3 ) )
			pf_version = PF_VERSION_20;
		if ( !strncmp ( buf, "2.1", 3 ) )
			pf_version = PF_VERSION_21;
		if ( !strncmp ( buf, "2.2", 3 ) )
			pf_version = PF_VERSION_22;
	}
	
	if ( pf_version == -1 ) {
		syslog ( LOGLEVEL, "pfqueue postfix2 backend: cannot determine postfix version (is postfix installed?)" );
	...
	p = popen ( pconf, "r" );
	if ( !p ) {
		syslog ( LOGLEVEL, "pfqueue postfix2 backend: cannot use postconf to search queue_directory, command was: \"%s\"", pconf );
		pclose ( p );
		return PFBE_UNUSABLE;
	}
...
int pfb_retr_headers( const char* msgid ) {
	...
	if ( has_configpath )
		snprintf ( buf, BUF_SIZE, "%s -c %s -q %s 2> /dev/null", 
			postcat_path, config_path, msg->id );
	else
...
int pfb_retr_status ( const char* msgid ) {
	...
	if ( CURQ == Q_DEFERRED ) {
		...
		if ( !p )
			...
			if ( pf_version > PF_VERSION_20 )
				flookfor ( p, msg->stat, sizeof(msg->stat), "reason=" );
			else
